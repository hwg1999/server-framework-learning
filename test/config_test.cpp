...
sylar::ConfigVar<int>::SPtr g_int_value_config = sylar::Config::Lookup( "system.port", 8080, "sysytem port" );

sylar::ConfigVar<float>::SPtr g_float_value_config
  = sylar::Config::Lookup( "system.value", 10.2f, "sysytem value" );

sylar::ConfigVar<std::vector<int>>::SPtr g_int_vec_value_config
  = sylar::Config::Lookup( "system.int_vec", std::vector<int> { 1, 2 }, "system int vec" );

sylar::ConfigVar<std::list<int>>::SPtr g_int_list_value_config
  = sylar::Config::Lookup( "system.int_list", std::list<int> { 1, 2 }, "system int list" );

sylar::ConfigVar<std::set<int>>::SPtr g_int_set_value_config
  = sylar::Config::Lookup( "system.int_set", std::set<int> { 1, 2 }, "system int set" );

sylar::ConfigVar<std::unordered_set<int>>::SPtr g_int_uset_value_config
  = sylar::Config::Lookup( "system.int_uset", std::unordered_set<int> { 1, 2 }, "system int uset" );

sylar::ConfigVar<std::map<std::string, int>>::SPtr g_str_int_map_value_config
  = sylar::Config::Lookup( "system.str_int_map", std::map<std::string, int> { { "k", 2 } }, "system str int map" );

sylar::ConfigVar<std::unordered_map<std::string, int>>::SPtr g_str_int_umap_value_config
...
void print_yaml( const YAML::Node& node, int level )
{
  if ( node.IsScalar() ) {
    ...
    SYLAR_LOG_INFO( SYLAR_LOG_ROOT() ) << std::string( level * 4, ' ' ) << "NULL - " << node.Type() << " - "
                                       << level;
  } else if ( node.IsMap() ) {
...
#define XX_M( g_var, name, prefix )                                                                                \
  ...
  YAML::Node root = YAML::LoadFile( "./config/config_test.yml" );
...
sylar::ConfigVar<Person>::SPtr g_person = sylar::Config::Lookup( "class.person", Person(), "system person" );

sylar::ConfigVar<std::map<std::string, Person>>::SPtr g_person_map
  = sylar::Config::Lookup( "class.map", std::map<std::string, Person>(), "system person" );

sylar::ConfigVar<std::map<std::string, std::vector<Person>>>::SPtr g_person_vec_map
